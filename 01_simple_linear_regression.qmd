...
The `se = FALSE` is included to stop ggplot from drawing confidence interval bands around the regression line. And, the `formula = y~x` is included simply to prevent ggplot from printing an annoying message that says <span style="font-color="red><code>`geom_smooth()` using formula 'y ~ x'</code></span> when creating the plot.